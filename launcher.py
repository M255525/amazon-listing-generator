# -*- coding: utf-8 -*-
"""亞馬遜5點式產品說明產生器 — 桌面啟動器（備用，本次未打包 exe）

執行時在本機 127.0.0.1:8805 啟動靜態伺服器並開啟預設瀏覽器。
固定使用 8805 埠，讓瀏覽器 localStorage（已產生的內容、API 設定、序號）跨次啟動保留。
"""
import http.server
import os
import socket
...
def main() -> None:
    ...
    if port_in_use():
        print("偵測到亞馬遜5點式產品說明產生器已在執行中，開啟瀏覽器頁面…")
        webbrowser.open(url)
        return
